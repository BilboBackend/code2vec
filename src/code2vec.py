...
if __name__ == "__main__":
    ...
    if config.SAVE_T2V is not None:
        ...
        config.log(
            "Target word vectors saved in word2vec text format in: %s" % config.SAVE_T2V
        )
        ...
        if eval_results is not None:
            config.log(
                str(eval_results).replace(
                    "topk",
                    "top{}".format(config.TOP_K_WORDS_CONSIDERED_DURING_PREDICTION),
                )
            )
    if config.PREDICT:
        ...
        print("Predicting")
        code_vector = predictor.one_time_predict(config.INPUTFILE)
        print(f"Code vector: {code_vector}")
