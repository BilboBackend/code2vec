...
class Code2VecModel(Code2VecModelBase):
    def __init__(self, config: Config):
        ...
        (
            self.predict_top_words_op,
            self.predict_top_values_op,
            self.predict_original_names_op,
        ) = None, None, None
    ...
    def train(self):
        ...
        num_batches_to_save_and_eval = max(
            int(self.config.train_steps_per_epoch * self.config.SAVE_EVERY_EPOCHS), 1
        )
        ...
        self.log(
            "Number of trainable params: {}".format(
                np.sum(
                    [
        ...
        for variable in tf.compat.v1.trainable_variables():
            self.log(
                "variable name: {} -- shape: {} -- #params: {}".format(
                    variable.name,
                    variable.get_shape(),
        ...
        try:
            while True:
                ...
                if batch_num % num_batches_to_save_and_eval == 0:
                    ...
                    model_save_path = (
                        self.config.MODEL_SAVE_PATH + "_iter" + str(epoch_num)
                    )
                    self.save(model_save_path)
                    self.log(
                        "Saved after %d epochs in: %s" % (epoch_num, model_save_path)
                    )
                    ...
                    self.log(
                        "After {nr_epochs} epochs -- {evaluation_results}".format(
                            nr_epochs=epoch_num,
    ...
    def evaluate(self) -> Optional[ModelEvaluationResults]:
        ...
        if self.eval_reader is None:
            ...
            (
                ...
                _,
                self.eval_code_vectors,
            ) = self._build_tf_test_graph(input_tensors)
        ...
        with open("log.txt", "w") as log_output_file:
            ...
            subtokens_evaluation_metric = SubtokensEvaluationMetric(
                partial(
                    common.filter_impossible_names,
    ...
    def _build_tf_training_graph(self, input_tensors):
        ...
        input_tensors = _TFTrainModelInputTensorsFormer().from_model_input_form(
            input_tensors
        )
        ...
        with tf.compat.v1.variable_scope("model"):
            ...
            attention_param = tf.compat.v1.get_variable(
                "ATTENTION", shape=(self.config.CODE_VECTOR_SIZE, 1), dtype=tf.float32
            )
            ...
            batch_size = tf.cast(
                tf.shape(input_tensors.target_index)[0], dtype=tf.float32
            )
            loss = (
                tf.reduce_sum(
    ...
    def _calculate_weighted_contexts(
        self,
        tokens_vocab,
        paths_vocab,
        attention_param,
        ...
        source_word_embed = tf.nn.embedding_lookup(
            params=tokens_vocab, ids=source_input
        )  # (batch, max_contexts, dim)
        path_embed = tf.nn.embedding_lookup(
            params=paths_vocab, ids=path_input
        )  # (batch, max_contexts, dim)
        target_word_embed = tf.nn.embedding_lookup(
            params=tokens_vocab, ids=target_input
        )  # (batch, max_contexts, dim)

        context_embed = tf.concat(
            [source_word_embed, path_embed, target_word_embed], axis=-1
        )  # (batch, max_contexts, dim * 3)

        if not is_evaluating:
            context_embed = tf.nn.dropout(
                context_embed, rate=1 - self.config.DROPOUT_KEEP_RATE
            )

        flat_embed = tf.reshape(
            context_embed, [-1, self.config.context_vector_size]
        )  # (batch * max_contexts, dim * 3)
        ...
        flat_embed = tf.tanh(
            tf.matmul(flat_embed, transform_param)
        )  # (batch * max_contexts, dim * 3)
        ...
        attention_weights = tf.nn.softmax(
            batched_contexts_weights, axis=1
        )  # (batch, max_contexts, 1)
    ...
    def _build_tf_test_graph(self, input_tensors, normalize_scores=False):
        with tf.compat.v1.variable_scope(
            ...
            input_tensors = _TFEvaluateModelInputTensorsFormer().from_model_input_form(
                input_tensors
            )
        ...
        return (
            ...
            original_words,
            attention_weights,
            input_tensors.path_source_token_strings,
            input_tensors.path_strings,
    ...
    def predict(
        ...
        for line in predict_data_lines:
            (
                ...
                batch_path_source_strings,
                batch_path_strings,
                batch_path_target_strings,
                batch_code_vectors,
            ) = self.sess.run(
                [
                    self.predict_top_words_op,
            ...
            prediction_results.append(
                ModelPredictionResults(
                    ...
                    code_vector=(
                        code_vectors if self.config.EXPORT_CODE_VECTORS else None
                    ),
                )
            )
    ...
    def _trace_training(self, sum_loss, batch_num, multi_batch_start_time):
        ...
        avg_loss = sum_loss / (
            self.config.NUM_BATCHES_TO_LOG_PROGRESS * self.config.TRAIN_BATCH_SIZE
        )
        throughput = (
...
class SubtokensEvaluationMetric:
    ...
    def update_batch(self, results):
        for original_name, top_words in results:
            ...
            self.nr_true_positives += sum(
                count
                for element, count in predicted_subtokens.items()
            ...
            self.nr_false_negatives += sum(
                count
                for element, count in original_subtokens.items()
    ...
    def recall(self):
        return self.nr_true_positives / (
            self.nr_true_positives + self.nr_false_negatives
        )
...
class TopKAccuracyEvaluationMetric:
    def __init__(self, top_k: int, get_first_match_word_from_top_predictions_fn):
        ...
        self.nr_predictions: int = 0
        self.get_first_match_word_from_top_predictions_fn = (
            get_first_match_word_from_top_predictions_fn
        )

    def update_batch(self, results):
        for original_name, top_predicted_words in results:
            self.nr_predictions += 1
            found_match = self.get_first_match_word_from_top_predictions_fn(
