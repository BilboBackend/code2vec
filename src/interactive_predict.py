...
MAX_PATH_LENGTH = 8
MAX_PATH_WIDTH = 2
JAR_PATH = "processing_tools/code2vec/JavaExtractor/JPredict/target/JavaExtractor-0.0.1-SNAPSHOT.jar"
...
class InteractivePredictor:
    ...
    def one_time_predict(self, input_filename):
        predict_lines, hash_to_string_dict = self.path_extractor.extract_paths(
            input_filename
        )
        ...
        # The model prediction returns a code vector for each method.
        # We use mean pooling to get the unweighted semantic mean.
        code_vector = np.mean([p.code_vector for p in raw_prediction_results], axis=0)
